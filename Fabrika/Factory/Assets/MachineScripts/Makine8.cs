...
public class Makine8 : MonoBehaviour
{
    ...
    IEnumerator getData()
    {
        ...
        else
        {
            ...
            // Debug.Log(lenght);
            title = stringList.ElementAt(36).Trim(' ').Trim('"');
            cont += "Durumu: " + stringList.ElementAt(37).Trim(' ').Trim('"') + "\n";
            cont += stringList.ElementAt(38).Trim(' ').Trim('"') + "\n";
            cont += "Ham madde : " + stringList.ElementAt(39).Trim(" ] ".ToCharArray()).Trim('"');
    ...
    void Start()
    {
        ...
        string query = "SELECT* FROM \"makine\" WHERE id=8";

...
public class Makine9 : MonoBehaviour
{
    ...
    IEnumerator getData()
    {
        ...
        else
        {
            ...
            // Debug.Log(lenght);
            title = stringList.ElementAt(41).Trim(' ').Trim('"');
            cont += "Durumu: " + stringList.ElementAt(42).Trim(' ').Trim('"') + "\n";
            cont += stringList.ElementAt(43).Trim(' ').Trim('"') + "\n";
            cont += "Ham madde : " + stringList.ElementAt(44).Trim(" ] ".ToCharArray()).Trim('"');
    ...
    void Start()
    {
        ...
        string query = "SELECT* FROM \"makine\" WHERE id=9";

...
public class Makine3 : MonoBehaviour
{
    ...
    IEnumerator getData()
    {
        ...
        else
        {
            ...
            // Debug.Log(lenght);
            title = stringList.ElementAt(11).Trim(' ').Trim('"');
            cont += "Durumu: " + stringList.ElementAt(12).Trim(' ').Trim('"') + "\n";
            cont += stringList.ElementAt(13).Trim(' ').Trim('"') + "\n";
            cont += "Ham madde : " + stringList.ElementAt(14).Trim(" ] ".ToCharArray()).Trim('"');
    ...
    void Start()
    {
        ...
        string query = "SELECT* FROM \"makine\" WHERE id=3";
